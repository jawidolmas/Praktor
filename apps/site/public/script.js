...
(function initPipelineGraph() {
  ...
  function render() {
    ...
    phaseNodesEl.innerHTML = L.phaseNodes.map((f) =>
      `<div style="${f.ws}"><div style="min-width:150px;max-width:186px;padding:11px 15px 12px;background:var(--color-surface);border:1px solid var(--color-card-border);box-shadow:0 0 0 1px var(--color-accent-900),0 10px 28px rgba(0,0,0,.35);border-radius:var(--radius-md,8px);display:flex;flex-direction:column;gap:5px"><span style="font-family:${MONO};font-size:11px;letter-spacing:.14em;color:var(--color-card-kicker)">${esc(f.n)}</span><span style="font-family:var(--font-heading);font-weight:500;font-size:23px;line-height:1.05;letter-spacing:-.01em;color:var(--color-card-text)">${esc(f.name)}</span><span style="font-size:11.5px;letter-spacing:.06em;color:var(--color-card-muted)">${esc(f.meta)}</span></div></div>`
    ).join("");
